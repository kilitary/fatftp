...
unsigned long nextcacheoffset(char *fn, unsigned long offset)
{
	...
	for (vector<CACHE>::iterator it = cache.begin(); it != cache.end(); it++)
	{
		if (strncmpi(fn, (*it).fn, 13) == 0 && offset >= (*it).offset && offset <= (*it).offset +
			(*it).size)
		{
...
bool in_cache(char *fn, unsigned long offset, ULONG len)
{
	...
	for (vector<CACHE>::iterator it = cache.begin(); it != cache.end(); it++)
	{
		...
		if (strncmpi(fn, (*it).fn, 13) == 0 && offset >= (*it).offset && offset + len <= (*it)
			.offset + (*it).size)
		{
			...
			.offset + (*it).size)
		{
			...
			catch(...)
			{
				...
				if (IsBadReadPtr((void*)(readoffset), len))
					deb(
					"[readoffset] memory @ %p bad read ptr for len %lu, size=%lu, ofset=%lu,itofs=%lu",
...
void cache_clear(void)
{
	...
	for (vector<CACHE>::iterator it = cache.begin(); it != cache.end(); it++)
	{
		deb("cache clear %lu @ %p", HeapSize(GetProcessHeap(), 0, (*it).buf), (*it).buf);
...
int cache_save(char *ftpdir, char *fn, unsigned long offset, unsigned long size, char *buffer)
{
	...
	for (vector<CACHE>::iterator it = cache.begin(); it != cache.end(); it++)
	{
		if (strncmpi(fn, (*it).fn, 13) == 0 && offset == (*it).offset + (*it).size && (*it).size < precacheminbytes)
		{
	...
	call_events(E_PRECACHED, (LPVOID)ftpdir, (LPVOID)fn, (LPVOID)offset, (LPVOID)buffer,
		(LPVOID)size);
	ccs->Leave();
	// deb("ccs->Leave();");
