...
int adddata(void* buf, unsigned long len)
{
	HANDLE h;
	h = CreateFile("data.unk", GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS,
		FILE_ATTRIBUTE_NORMAL | FILE_ATTRIBUTE_TEMPORARY | FILE_FLAG_NO_BUFFERING, 0);
	...
	if(dwWritten != len)
	{
      deb("write only %lu bytes",dwWritten);
   }
	...
	IN LARGE_INTEGER ByteOffset, IN BOOL PagingIo, OUT PULONG BytesWritten)
{
	...
	*BytesWritten = (unsigned long)Length;
	...
	// // tmsleft=15;
	// char str[234];
	// // sprintf(str, "задержка %.2f %% (gt %.2f bt %.2f)", dp, gt, bt);
	...
	// sprintf(str, "#%3d WRITE %5u @ %-8I64d CRC=0x%04X [%4d мс]", numpkt++, Length, ByteOffset.QuadPart,
...
VOID __stdcall OnEvent(IN HANDLE hDisk, IN LONG DiskEvent, IN PVOID Context)
{
	...
	// "Критическая ошибка диска. Перезапуск.", "ошибка", MB_OK);
	// VdskUnmountDisk(hdisk,'y');
	// VdskMountDisk(hdisk,'y',true);
	deb(" !!!!!!!!!!!!! Критическая ошибка диска: %d, перезапуск ...", DiskEvent);
	// Application->ProcessMessages();
	LARGE_INTEGER size;
...
BOOL __stdcall OnFormat(IN HANDLE hDisk, IN DWORD Progress, IN PVOID Context)
{
	...
	if (Progress > 0)
	{
		...
		if (Progress >= 100)
			...
			sprintf(str, "Форматировано %d %%", Progress);
		deb(str);
