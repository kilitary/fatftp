...
int fataddfiles(char *dirname, LPVOID memory, unsigned long stclst, char *buf)
{
	...
	while (nl)
	{
		...
		// deb("scanning line ...");
		fsize = 0;
		ret = sscanf(line, "%10s %*lu %*s %*s %lu %s %s %s %[!a-zA-Z0-9~ \-а-€ј-я._]", &attrs,
			&fsize, &month, &day, &crtime, &fn);
		if (ret != 6 || strstr(line, ":"))
		{
			// deb("another scanf %d", ret);
			ret = sscanf(line, "%10s %*lu %*s %*s %lu %s %s %s %*s %[!a-zA-Z0-9~ \-а-€ј-я._]", &attrs,
				&fsize, &month, &day, &crtime, &fn);
		...
		// Bits 0Ц4: Day of month, valid value range 1-31 inclusive.
		// Bits 5Ц8: Month of year, 1 = January, valid value range 1Ц12 inclusive.
		// Bits 9Ц15: Count of years from 1980, valid value range 0Ц127 inclusive (1980Ц2107).
		...
		// Bits 0Ц4: 2-second count, valid value range 0Ц29 inclusive (0 Ц 58 seconds).
		// Bits 5Ц10: Minutes, valid value range 0Ц59 inclusive.
		// Bits 11Ц15: Hours, valid value range 0Ц23 inclusive.
		...
		deb(" #%-3d %25s %8u байт (%.2f MB) ", nfiles, filename, fsize, (double)fsize / 1024 / 1024);
		// stc = cluster;
		// fat[stc] = 0x0fffffff;
		// stc = getfreecluster();
		// fat[stc] = 0x0fffffff;
		...
		allocatedbytes = 0;
		// sprintf(sss, "%s\%s, %.2f кб", dirname + 1, filename, (double)((double)fsize / 1024.0));
		// Form1->knotts->Title->Text->Text = sss;
		while (ssize > 0)
		{
			unsigned long nclst;
			...
			if (entrcls)
			{
				...
				if ((GetTickCount() - tms2) >= 700)
				{
					pr = ((((double)allocatedbytes ? (double)allocatedbytes : (double)1)) / (double)
						dir.size) * (double)100;
					sprintf(sss, "запись FAT %.2f %%[%s\%s]", pr, dirname, filename);
		...
		// char ffn[1024];
		// sprintf(ffn, "y:\\%s\\%s", dirname + 1, filename);
		// sprintf(sss, "%-50s (%.2f Mб, %6lu clstrs from %5lu)", ffn,
...
bool nconn(void)
{
	...
	deb("соединение с сервером ...");
	// Form1->Canvas->Font->Color=clRed;
	// Form1->Canvas->Font->Size=12;
	// Form1->deblog->Canvas->TextOutA(Form1->Left+Form1->deblog->Width/2,Form1->deblog->Height/2,"соединение ...");
	...
	{
		deb("Client: невозможно соединица с сервером\n");
		return false;
	}
	ms = GetTickCount() - ms;
	deb("    за %d мсек\n", ms);
...
int ftpauth(void)
{
	...
	// Form1->knotts->Title->Text->Text = "логин ...";
	...
	{
		// deb("ошибка аутентификации: %s", buf);
		// ftpcmd("STAT\r\n", 0, 0);
		// ftprl(buf, timeoutst, 0);
...
bool ftpconn(bool quick = false)
{
	...
	if (strlen(buf) < 2)
	{
		...
		closesocket(FtpSocket);
		FtpSocket = 0;
		call_events(E_FTPDISCONNECT, (LPVOID)"нет кода приветстви€ 220!");
		return false;
	}
...
void showfat(void)
{
	...
	deb("memory @ 0x%08p, disk: использовано %lu MB из %I64d (%I64d GB)", memory,
	...
	if (fb.sign != 0xaa55)
	{
		deb("сигнатура FAT отсутствует, copy %d", 4096);
		return;
	}
	...
	deb("размер FAT = %d (%d MB)", fatsize, fatsize / 1024 / 1024);
...
void makefatdisk(void)
{
	...
	// if (nl[i] == 0xa)
	// nb++;
	// deb(" FAT:загружено %d записей: \r\n%s", nb, temp3);
	DWORD pfat = 0; // 0xab020;
	fatsize = fat32boot.sectors_per_fat * fat32boot.bytes_per_sector;
	// deb("размер FAT = %lu (%lu MB в %d секторах)", fatsize, fatsize/1024/1024,
	...
	if (fatsize > curdisksize)
	{
		...
		deb("фат  %d ћЅ @ %p", ns / 1024 / 1024, memory);
	...
	// deb("FAT структуры занимают %lu байт (%.2f MB)", (unsigned long)pfat,
	...
	deb("кластер = %lu байт, всего байт = %lu (%.2f GB)\r\кластеров = %lu", clstsize,
	...
	pData = (LPVOID)((unsigned int)memory + dataoffset);
	deb("1 кластер @ %p (dataoffset %d, 0x%4X)", pData, dataoffset, dataoffset);
	...
	// deb("пам€ть дл€ фат 2 таблицы %d ћЅ", ns/1024/1024);
	// memory = HeapReAlloc(GetProcessHeap(), 0, memory, ns);
	// curdisksize = ns;
	//
	// }
	...
	deb("NetFAT: %d, vol: 1, FAT: %d. rootdirsize: %lu байт", files.size(), added,
		(unsigned long)(added * 2 + 1)*sizeof(DIR));
	// deb("копируем fat1 = > %d fat2 = > %d (%d bytes) memory @ %p", pfat1, pfat2,
	...
	try
	{
		...
		// maxquad = totalmb*1.7;
		// Form1->current->Text = "FAT диск создан";
		// Form1->Repaint();
	}
	catch(...)
	{
...
int mountdisk(void)
{
	...
	else
	{
		...
		deb("ƒиск Y:\\ (%lu GB)  подключен к %s.",
	...
	char *dosfn, unsigned long cacheblocks, unsigned long startoffset)
{
	...
	deb("ftppasvread() = read %d, numr: %lu ss %d, %lu мс, numrecvs: %lu,sleeps:%lu", readok, numr,
